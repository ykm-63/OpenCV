...
```cmd
python -m ultralytics yolo detect train model=yolov8n.pt data=dataset/data.yaml epochs=30 imgsz=640
```
...
```cmd
"C:\Users\sdsd4\.cache\codex-runtimes\codex-primary-runtime\dependencies\python\python.exe" -c "from ultralytics import YOLO; model = YOLO('yolov8n.pt'); model.train(data='dataset/data.yaml', epochs=30, imgsz=640)"
```
